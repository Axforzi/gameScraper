...
- `pyproject.toml` — package metadata + dependencies (PEP 621) + ruff/mypy/pytest tool config; dev tooling in `[dependency-groups] dev`
- `uv.lock` — resolved dependency graph generated by uv
- `.github/workflows/ci.yml` — lint + test jobs on push/PR, matrix 3.11–3.13, via uv
...
2. **Resolved: `.env` support** — `python-dotenv` is a production dependency and `main.py` calls `load_dotenv()` at startup (env vars win over the file). `.env` is gitignored; documented in README. Related cleanup: project migrated from pip+requirements.txt to uv (deps in `pyproject.toml`, `uv.lock` committed, CI uses `astral-sh/setup-uv`), `requires-python` bumped to `>=3.11` (ipython 9.2.0 needs it), ruff/mypy target py311.
