...
The system MUST source country, locale, and currency from environment variables or a config file, with per-store defaults kept as today's values so current behavior is unchanged when unset: Steam/EGS default to country `VE`, locale `es-ES`, currency `USD`, and GOG defaults to its own country `US`, locale `en-US`, currency `USD`.

#### Scenario: Defaults preserve current behavior

- GIVEN no config variables set
- WHEN the app starts
- THEN Steam/EGS use country=VE, locale=es-ES, currency=USD
- AND GOG uses country=US, locale=en-US, currency=USD

#### Scenario: Override via environment

- GIVEN COUNTRY=US, LOCALE=en-US, CURRENCY=EUR are exported (and optionally GOG_COUNTRY/GOG_LOCALE/GOG_CURRENCY)
...
- Tier 2; values extracted: VE/es-ES/USD (Steam/EGS) + US/en-US/USD (GOG), SECRET_KEY, Waitress host/port, debug mode, DOWNLOAD_DELAY, spider timeout.
